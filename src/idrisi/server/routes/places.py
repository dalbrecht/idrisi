...
def _get_service(request: Request) -> PlaceService:
    from idrisi.application.place_service import PlaceService as _PlaceService  # noqa: PLC0415
    from idrisi.infrastructure.db.repository import SqlPlaceRepository  # noqa: PLC0415
    from idrisi.infrastructure.geocoding.nominatim import (  # noqa: PLC0415
...
def create_places_router() -> APIRouter:
    ...
    def delete_place(request: Request, place_id: str) -> Response:
        from idrisi.domain.errors import BadRequestError, PlaceNotFoundError  # noqa: PLC0415

        service = _get_service(request)
        try:
            entity_id = uuid.UUID(place_id)
        except ValueError:
            raise BadRequestError(f"Invalid UUID: {place_id}") from None
