...
def _get_service(request: Request) -> TripService:
    from idrisi.application.trip_service import TripService as _TripService  # noqa: PLC0415
    from idrisi.infrastructure.db.repository import SqlTripRepository  # noqa: PLC0415
...
def create_trips_router() -> APIRouter:
    ...
    def delete_trip(request: Request, trip_id: str) -> Response:
        from idrisi.domain.errors import BadRequestError, TripNotFoundError  # noqa: PLC0415

        service = _get_service(request)
        try:
            entity_id = uuid.UUID(trip_id)
        except ValueError:
            raise BadRequestError(f"Invalid UUID: {trip_id}") from None
