---
title: "CLI Workflow"
description: "End-to-end walkthrough of a realistic Idrisi CLI session"
section: "guides"
order: 1
---
...
This guide walks through a complete Idrisi workflow from importing photos to rendering a finished map, using the CLI.
...
```bash
idrisi import photos ~/Photos/europe-2025 --trip "Europe 2025" --dry-run
```

This scans the directory and reports what would be imported without writing anything to the database. Review the output to confirm Idrisi found the expected files and extracted locations correctly.

When satisfied, run without `--dry-run` to import:

```bash
idrisi import photos ~/Photos/europe-2025 --trip "Europe 2025"
```

**What happens during import:**
Idrisi reads EXIF metadata from each photo in the top-level of the directory (subdirectories are not scanned). Photos with GPS coordinates are saved as `Photo` records. Photos without GPS data are skipped entirely — no record is created for them. No `Place` records are created automatically during import.
...
```bash
idrisi place add --name "Eiffel Tower" --lat 48.8584 --lon 2.2945 --category landmark
```
...
```bash
idrisi place search "Colosseum"
```

Idrisi queries the Nominatim geocoding service and returns matching results with coordinates. Select the correct result to add it to your database. This is useful for well-known places where you don't want to look up coordinates yourself.
...
```bash
idrisi trip create "Italy 2025" --description "Two weeks in Italy"
```
...
```bash
idrisi project create "Italy Map" --map-type route
```
...
```bash
idrisi render "Italy Map" --style vintage --format svg --dpi 300 --output ./maps
```

Idrisi renders the project using the specified style and writes the output file to `./maps`.
...
```bash
idrisi render "Italy Map" --format pdf --dpi 300 --output ./maps
```

Dark style for screen use:

```bash
idrisi render "Italy Map" --style dark --format png --output ./maps
```
